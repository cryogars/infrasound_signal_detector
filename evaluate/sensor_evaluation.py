...
if __name__ == "__main__":
    ...
    # load model
    logger.debug(f"Evaluating Sensor Agnostic Model")
    logger.debug(f"Loading Model")
    the_model_path = f"sensor_model.pth"
    ...
    evaluate(
        ...
        x_paths= f"{config.PROCESSED_DATA}/two_high_signal/high_signal_X_train_.parquet",
        y_paths= f"{config.PROCESSED_DATA}/two_high_signal/high_signal_y_train_.parquet",
        cm_plot_title= rf"$MC_1$ Confusion Matrix for Train set",
        cm_save_path= "mc1_train_confusion_matrix",
    )
    ...
    evaluate(
        model=the_model,
        the_transformation=config.TRANSFORMATION,
        x_paths=f"{config.PROCESSED_DATA}/two_high_signal/high_signal_X_test_.parquet",
        y_paths=f"{config.PROCESSED_DATA}/two_high_signal/high_signal_y_test_.parquet",
        cm_plot_title=rf"$MC_1$ Confusion Matrix for Validation set",
        cm_save_path="mc1_validation_confusion_matrix",
    )
        ...
        for sensor in sensors:
            if not (station == "LCC2" and sensor == 3):
                ...
                evaluate(
                    ...
                    x_paths=X_path,
                    y_paths=y_path,
                    cm_plot_title=rf"$MC_1$ Confusion Matrix: Station {station} Sensor: {sensor}",
                    cm_save_path=f"mc1_cm_station{station}_sensor{sensor}",
                )
